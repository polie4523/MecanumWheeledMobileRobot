...
void MecanumWMR::setPIDgain_translation(const float kp, const float ki, const float kb) {
...
void MecanumWMR::setPIDgain_rotation(const float kp, const float ki, const float kb) {
...
bool MecanumWMR::Point2PointMove(const std::array<float, 3> target_point) {
    ...
    angle = atan2f(target_point[1]-starting_point[1], target_point[0]-starting_point[0]);
    //std::cout<<"dist : "<<dist<<std::endl;
    //std::cout<<"angle : "<<angle*180/PI<<std::endl;
    if (!tg.generateSCurve(dist)) {
        std::cout<<"S-curve generation fail"<<std::endl;
        return false;
    }
	i2c.EnableSTM32();
	timer_start();
...
void MecanumWMR::pos_controller(const std::array<float, 3> p_ref, const std::array<float, 3> v_ref) {
    ...
    for (int i=0;i<3;i++) {
        ...
        v_global[i] = v_ref[i] + ControllerParam.kp[i]*error + ControllerParam.ki[i]*sum_error[i];
...
void MecanumWMR::timer_callback(void) {
    ...
    std::array<float, 3> p_ref;
    std::array<float, 3> v_ref;
    if (!(isDone = tg.getTrajectory(traj))) {
        v_ref[0] = traj[3]*cos(angle);
        v_ref[1] = traj[3]*sin(angle);
        v_ref[2] = 0;
        p_ref[0] = starting_point[0] + traj[4]*cos(angle);
        p_ref[1] = starting_point[1] + traj[4]*sin(angle);
        p_ref[2] = 0;
        ...
        if (log) Log2File(p_ref,g,m);
    }
}

void MecanumWMR::Log2File(const std::array<float, 3> p_ref, float g, const std::array<float, 3> m) {
    for (short i=0;i<3;i++) fprintf(p_fptr, "%.3f ", p_est[i]);
    for (short i=0;i<3;i++) {
        if (i<2) fprintf(p_fptr, "%.3f ", p_ref[i]);
        else fprintf(p_fptr, "%.3f %.3f %.3f %.3f %.3f\n", p_ref[i], g, m[0], m[1], m[2]);
    }
