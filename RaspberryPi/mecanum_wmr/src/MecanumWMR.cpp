...
void MecanumWMR::timer_callback(void) {
    ...
    if (!(isDone = tg.getTrajectory(traj))) {
        /*std::cout<<"time : "<<traj[0]<<"\t";
        std::cout<<"jerk : "<<traj[1]<<"\t";
        std::cout<<"acc : "<<traj[2]<<"\t";
        std::cout<<"vel : "<<traj[3]<<"\t";
        std::cout<<"pos : "<<traj[4]<<std::endl;*/
        ...
        if (log) Log2File(p_ref);
    }
}

void MecanumWMR::Log2File(const std::array<float, 3> p_ref) {
    for (short i=0;i<3;i++) fprintf(p_fptr, "%.3f ", p_est[i]);
    for (short i=0;i<3;i++) {
        if (i<2) fprintf(p_fptr, "%.3f ", p_ref[i]);
        else fprintf(p_fptr, "%.3f\n", p_ref[i]);
    }
